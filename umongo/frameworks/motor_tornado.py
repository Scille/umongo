import asyncio
from motor.motor_tornado import MotorDatabase, MotorCursor
...
class WrappedCursor(MotorCursor):
...
class MotorTornadoDocument(DocumentImplementation):
...
class MotorTornadoReference(Reference):
...
class MotorTornadoBuilder(BaseBuilder):

    BASE_DOCUMENT_CLS = MotorTornadoDocument

    @staticmethod
    def is_compatible_with(db):
        return isinstance(db, MotorDatabase)
    ...
    def _patch_field(self, field):
        ...
        if isinstance(field, ReferenceField):
            ...
            field.reference_cls = MotorTornadoReference
